        ...
        <div class="row justify-content-center align-items-center">
            <div class="col-12 col-md-6">
                <form class="bg-body-tertiary shadow-lg rounded-4 p-5 text-black" action="{{ route('become.revisor') }}" method="POST">
                    ...
                    <div class="mb-3">
                        <label for="registerEmail" class="form-label">{{__('ui.talkaboutyou')}} </label>
                        <div class="input-group">
                            <div class="import-group-addon border border-1 border-body-tertiary rounded-start-2">
                            </div>
                            <textarea rows="10" type="text" class="form-control @error('name') is-invalid @enderror"
                                id="registerEmail" name="email"></textarea>
                            @error('name')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
